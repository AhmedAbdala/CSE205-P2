 ...
 *
 * DESCRIPTION
 * This is the driver program, it parses an input file p2-students.txt and writes the output into
 * p2-tuition.txt, the output is arranged ascendingly according to the student ID and it also
 * has the tuition fees incurred by each student.
 *
...
public class Main {
     ...
     *
     * -- In the try-catch block we try to read the list of students from p2-students-1.txt
     ...
     *     Print "Sorry, could not open 'p2-students-1.txt' for reading. Stopping."
     *     Call System.exit(-1)
     * end try-catch
     *
    ...
    public void run () {
        ...
        /**
        /*This format can't be used as it is neither a primitive data type, nor
         *is it a wrapper class. It has to be initialized fully using the new keyword.
         */
    ...
    /**
     * readFile() Reads the student information from "p2-students.txt" and returns the list of students as
     ...
     * Open "p2-students-1.txt" for reading using a Scanner object named in
     ...
     * Return student
     */
    public OnlineStudent readOnlineStudent (Scanner pIn) {
    ...
    public void writeFile (ArrayList<Student> pStudentList) throws FileNotFoundException {
        ...
        File outFile = new File("p2-tuition.txt");
        PrintWriter out = new PrintWriter(outFile);

        for (Student in: pStudentList) {
            out.printf("%-16s %-20s %-15s %8.2f", in.getId(), in.getLastName(),
                    in.getFirstName(), in.getTuition());
            out.println();
        }
        out.close();
    }
